/**
 * SQLite schema for LMF4.1
 *
 * Preserves existing SQLite DDL (FTS5, sqlite-vec, bun:sqlite pragmas).
 * Fixes applied vs original schema.ts:
 *   - fabric_extract column intentionally preserved (back-compat) in loa_entries + loa_fts + loa triggers
 *   - message_range_start/end FKs now have ON DELETE SET NULL
 */
...
-- FIXED: fabric_extract column intentionally preserved (back-compat)
CREATE TABLE IF NOT EXISTS loa_entries (
  ...
  message_range_start INTEGER REFERENCES messages(id) ON DELETE SET NULL,
  message_range_end   INTEGER REFERENCES messages(id) ON DELETE SET NULL,
...
export const SQLITE_CREATE_INDEXES = `
...
// fabric_extract: correct name per upstream 4.1.0 back-compat decision
export const SQLITE_CREATE_FTS = `
...
CREATE VIRTUAL TABLE IF NOT EXISTS loa_fts USING fts5(
  title, description, fabric_extract, tags, project,
...
// loa triggers: fabric_extract column name is intentional
export const SQLITE_CREATE_FTS_TRIGGERS = `
...
CREATE TRIGGER IF NOT EXISTS loa_ai AFTER INSERT ON loa_entries BEGIN
  INSERT INTO loa_fts(rowid, title, description, fabric_extract, tags, project)
  VALUES (new.id, new.title, new.description, new.fabric_extract, new.tags, new.project);
END;
CREATE TRIGGER IF NOT EXISTS loa_ad AFTER DELETE ON loa_entries BEGIN
  INSERT INTO loa_fts(loa_fts, rowid, title, description, fabric_extract, tags, project)
  VALUES('delete', old.id, old.title, old.description, old.fabric_extract, old.tags, old.project);
END;
...
export const SQLITE_CREATE_VECTOR_TABLES = `
...
/** Ordered list of DDL strings for SQLiteAdapter.init() */
export const SQLITE_SCHEMA_ORDERED = [
  SQLITE_CREATE_TABLES,
  SQLITE_CREATE_INDEXES,
  SQLITE_CREATE_FTS,
  SQLITE_CREATE_FTS_TRIGGERS,
  SQLITE_CREATE_VECTOR_TABLES,
];
